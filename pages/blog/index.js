...
export default function BlogIndex({ postsArr }) {
    ...
    const pageNumbers = parsedPosts.map((arr, index) => {
        return page !== index ? <p className={styles.page_number} key={index} onClick={() => pageChange(index)}>{index + 1}</p> :
            <p className={styles.page_number_selected} key={index} onClick={() => pageChange(index)}>{index + 1}</p>
    })

    const parsedPostsArr = parsedPosts[page].map((post) => {
            ...
            <div className={styles.main_container}>
                <div className={styles.post_container}>
                    ...
                    <div className={styles.nav_container}>
                        <p className={styles.page_word}>Page</p>
                        {pageNumbers}
                    </div>
                </div>
                <div className={homeStyles.side_container} style={{ marginLeft: "5vw" }}>
...
export function getStaticProps() {
    ...
    while (postsArr.length > 0) {
        holderArr.push(postsArr.splice(0, 4))
    }

    return {
        props: {
            postsArr: JSON.stringify(holderArr)
        }
    }
